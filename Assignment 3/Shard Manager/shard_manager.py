...
def primary_elect():
    ...
    server_id_to_shard = requests.get("http://lb:5000/get_server_id_to_shard").json()
    servers_to_hostname = requests.get("http://lb:5000/get_servers_to_hostname").json()
    servers = []
    ...
    for server in servers:
        response = requests.post(f"http://{servers_to_hostname[server]}:5000/wal", json={"shard": shard_id})
    ...
    try:
        ...
        response = requests.post(f"http://{servers_to_hostname[primary_server]}:5000/primary_elect", json={"shard": shard_id, "secondary_servers": secondary_servers})
...
def update_primary_if_required():
    ...
    for shard_id, server_id in PRIMARY_SERVERS.items():
        if server_id in deleted_servers:
            response = requests.post("http://sm:5000/primary_elect", json={"shard_id": shard_id})
    ...
    while True:
        ...
        for server_id, hostname in servers_to_hostname.items():
            ...
            if response.status_code != 200:
                ...
                requests.post("http://lb:5000/remove_metadata", json={"server_id": server_id})
                requests.post("http://sm:5000/update_primary_if_required", json={"deleted_servers": [server_id]})
                
        # Spawn new servers for shards of dead servers
        requests.post("http://lb:5000/spawn_servers", json={"shard_ids_for_new_servers": shard_ids_for_new_servers})
